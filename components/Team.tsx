    ...
    <section className="bg-white py-32 px-4">
      <div className="max-w-[1400px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 px-4">
            <h2 className="text-5xl md:text-7xl font-bold text-slate-900 font-[Syne] tracking-tight leading-[0.9]">
                Meet The <br />
                <span className="text-teal-600">Visionaries.</span>
            </h2>
            <p className="max-w-md text-slate-500 text-lg mt-6 md:mt-0 text-right">
                The creative minds and strategic thinkers driving the revolution in business consultancy.
            </p>
        </div>
                ...
                >
                    ...
                    <img 
                        ...
                        className={`absolute inset-0 w-full h-full object-cover transition-transform duration-1000 ${
                            activeId === member.id ? 'scale-100' : 'scale-150 grayscale'
                        }`}
                    />
                    ...
                    <div className={`absolute bottom-0 left-0 w-full p-8 md:p-12 transition-all duration-500 transform ${
                        ...
                        <div className="bg-white/10 backdrop-blur-md border border-white/10 p-6 rounded-2xl max-w-md">
                            <span className="inline-block px-3 py-1 rounded-full bg-teal-500 text-white text-xs font-bold uppercase tracking-wider mb-4">
                                {member.role}
                            </span>
                            ...
                            <div className="flex gap-4">
                                <button className="w-10 h-10 rounded-full bg-white text-slate-900 flex items-center justify-center hover:bg-teal-500 hover:text-white transition-colors">
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${
                        activeId === member.id ? 'opacity-0 pointer-events-none' : 'opacity-100'
                         ...
                         <h3 className="text-4xl font-bold text-white opacity-50 whitespace-nowrap lg:-rotate-90 tracking-widest uppercase hidden lg:block">
                            {member.name.split(' ')[0]}
                         </h3>
                         <div className="lg:hidden absolute bottom-8 left-8 text-white">
                             <h3 className="text-2xl font-bold">{member.name}</h3>
                             <p className="text-teal-400 text-sm font-bold uppercase">{member.role}</p>
                         </div>
