...
const Pricing: React.FC = () => {
  return (
    <section className="bg-slate-50 py-32 px-4 relative overflow-hidden">
      {/* Ambient Background */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-teal-100/50 rounded-full blur-[100px] pointer-events-none mix-blend-multiply"></div>
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-purple-100/50 rounded-full blur-[100px] pointer-events-none mix-blend-multiply"></div>
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        <div className="text-center mb-20">
            <span className="text-teal-600 font-bold tracking-[0.2em] text-xs uppercase mb-4 block">
                Unlock Potential
            </span>
            <h2 className="text-5xl md:text-7xl font-bold text-slate-900 font-[Syne] mb-6">
                Investment <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-500 to-purple-600">Models</span>
            </h2>
            <p className="text-slate-500 text-lg max-w-2xl mx-auto">
                Transparent partnership structures designed to scale with your ambition. Choose the velocity that fits your vision.
            </p>
        </div>
            ...
            {PRICING_PLANS.map((plan, index) => (
                ...
                >
                    ...
                    <div className={`h-full relative rounded-[2.5rem] p-1 overflow-hidden transition-transform duration-500 hover:-translate-y-2 ${
                        plan.highlight
                        ? 'bg-gradient-to-b from-teal-500 via-slate-900 to-slate-900 shadow-2xl shadow-teal-900/20'
                        : 'bg-white border border-slate-200 hover:border-teal-300 shadow-xl'
                    }`}>
                        ...
                        <div className={`h-full rounded-[2.3rem] p-8 md:p-10 flex flex-col relative overflow-hidden ${
                            ...
                            <div className="flex justify-between items-start mb-8 relative">
                                ...
                                {plan.highlight && (
                                    <span className="px-4 py-1.5 rounded-full bg-teal-500/10 border border-teal-500/20 text-teal-400 text-[11px] font-bold uppercase tracking-widest flex items-center gap-2">
                            ...
                            <div className="mb-8 relative">
                                ...
                                <p className={`mt-6 text-sm leading-relaxed border-l-2 pl-4 ${
                                    plan.highlight ? 'text-slate-400 border-teal-500/30' : 'text-slate-500 border-slate-200'
                                }`}>
                                    {plan.description}
                                </p>
                            </div>
                                ...
                                {plan.features.map((feature, idx) => (
                                    <li key={idx} className="flex items-start gap-4 group/item">
                                        <div className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center shrink-0 transition-colors ${
                                            plan.highlight 
                                            ? 'bg-teal-500/20 text-teal-400 group-hover/item:bg-teal-500 group-hover/item:text-white' 
                            ...
                            <button className={`w-full py-5 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 group/btn transition-all relative overflow-hidden ${
                                plan.highlight 
                                ? 'bg-gradient-to-r from-teal-500 to-teal-400 text-white shadow-lg shadow-teal-500/25 hover:shadow-teal-500/40' 
